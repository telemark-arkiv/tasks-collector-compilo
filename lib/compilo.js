  ...
  getFeedFromCompilo(user, function (error, feed) {
    if (error) {
      ...
      repackFeed(feed, function (err, data) {
        console.log(`tasks-collector-compilo: repacks feed for ${user}`)
        if (err) {
          console.log(`tasks-collector-compilo: repack failed for ${user} - ${JSON.stringify(err)}`)
        } else {
          console.log(`tasks-collector-compilo: finished collecting tasks for ${user} - found ${data.length}`)
          result.data = data
